...
async def hefeng(session, queue):
...
async def openweathermap(session, queue):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        asyncio.create_task(hefeng(session, queue))
        asyncio.create_task(openweathermap(session, queue))
        for _ in range(2):
